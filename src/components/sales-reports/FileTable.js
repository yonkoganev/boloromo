...
export default function FileTable({ files = [], loading, onDeleted, category }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/files?category=${category}&filename=${encodeURIComponent(
          deleteTarget.name
        )}`,
        { method: "DELETE" }
      );
  ...
  return (
    <>
              ...
              <TableRow key={file.name}>
                ...
                <TableCell align="right">
                  ...
                  <IconButton
                    ...
                    onClick={() =>
                      window.open(
                        `${API_URL}/download?category=${category}&filename=${encodeURIComponent(
                          file.name
                        )}`,
                        "_blank"
                      )
      ...
      {/* SNACKBAR (ALWAYS MOUNTED – SAME AS DIREX) */}
      <Snackbar
        open={snackbar.open}
